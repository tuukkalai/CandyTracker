...
def settings():
    ...
    if request.method == "POST":
        ...
        change = users.change_password(
            password, password2, oldPassword, tokenc)
    ...
    if not group_info:
        ...
        for c in active_challenges:
            group_challenges.append(
                [c.id, c.name, groups.get_no_of_users_still_in_the_race(group_id, c.id)])
        ...
        for ch in previous_challenges:
            prev_challenges.append(
                [ch.id, ch.name, groups.get_no_of_users_still_in_the_race(group_id, ch.id)])
